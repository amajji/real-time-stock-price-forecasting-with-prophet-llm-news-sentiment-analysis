...
class LstmArimaModels():
    def __init__(self, n_steps_ahead_value=1, time_step = 6):
        self.n_steps_ahead_value = n_steps_ahead_value
        self.time_step = time_step
    
    def create_sequences_multivariate(self, data_in):
        """ 
        This function reshapes data_in to be 3D as expected by LSTM (samples, time steps, features)
        """
        X_out = []
        y_out = []
        for i in range(self.time_step, len(data_in)-self.n_steps_ahead_value + 1):
            X_out.append(data_in[i-self.time_step:i, :])  # Take the previous `time_step` data as input (both Bitcoin and S&P 500)
            y_out.append(data_in[i:i+self.n_steps_ahead_value, 0])  # Predict the next n_steps_ahead Bitcoin price
        return np.array(X_out), np.array(y_out)

    def lstm_model(self, data_train, data_test, date, target, retrain=False):
        if retrain:
        ...
        X_train, y_train = self.create_sequences_multivariate(data_train_norm)

        # Create sequences with the test data (including both Bitcoin and S&P 500 features)
        X_test, y_test = self.create_sequences_multivariate(data_test_norm)
        ...
        if retrain:
            ...
            model.add(Dense(units=self.n_steps_ahead_value))  # Predict the next Bitcoin price
    ...
    def arima_model(self, data_train, data_test, date, target, retrain=False):
        if retrain:
